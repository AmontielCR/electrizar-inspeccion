...
═══════════════════════════════════════════ */
const MSAL_CONFIG = {
  clientId: "4565a5bc-5a7a-41df-86d4-ef7879fd78b0",
  tenantId: "7d0cd9ce-34e3-4da0-8084-93d6e1ae0a0a",
  redirectUri: window.location.origin,
  scopes: ["User.Read", "Files.ReadWrite", "offline_access"],
};

/* Simple MSAL-lite usando popup OAuth2 con PKCE */
const authHelper = {
  getAuthUrl: () => {
    ...
    return `https://login.microsoftonline.com/${MSAL_CONFIG.tenantId}/oauth2/v2.0/authorize?${params}`;
  },
  ...
  parseToken: () => {
    ...
    const exp = Date.now() + parseInt(expiresIn || "3600") * 1000;
    localStorage.setItem("ms_token", token);
    localStorage.setItem("ms_token_exp", exp.toString());
    return token;
  },

  getToken: () => {
    const token = localStorage.getItem("ms_token");
    const exp = parseInt(localStorage.getItem("ms_token_exp") || "0");
...
const oneDrive = {
  ensureFolder: async (token, path) => {
    ...
    for (const part of parts) {
      ...
      try {
        await fetch(
          `https://graph.microsoft.com/v1.0/me/drive/root:/${fullPath}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
      } catch {}
      try {
        const parent = current
          ? `https://graph.microsoft.com/v1.0/me/drive/root:/${current}:/children`
  ...
  uploadFile: async (token, path, content, isBase64 = false) => {
    let body;
    if (isBase64) {
    ...
    const r = await fetch(url, {
      ...
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": isBase64 ? "image/jpeg" : "application/json",
      },
      body,
    });
    return r.ok;
  ...
  saveReport: async (token, informe) => {
    const folder = `Electrizar-Reportes/${informe.codigo}-${(informe.propietario || "").replace(/[^a-zA-Z0-9]/g, "_").slice(0, 20)}`;
    await oneDrive.ensureFolder(token, folder);
    // Guardar datos JSON
    await oneDrive.uploadFile(token, `${folder}/datos.json`, informe);
    // Guardar fotos
    for (const el of informe.elementos || []) {
      if (el.url) {
        const name = `foto_${String(el.num).padStart(2, "0")}_${el.titulo.replace(/[^a-zA-Z0-9]/g, "_").slice(0, 30)}.jpg`;
        await oneDrive.uploadFile(token, `${folder}/${name}`, el.url, true);
      }
    }
    return folder;
  },
    ...
    <div className="space-y-2">
      ...
      {items.map((h, i) => (
        <div key={i} className="flex gap-2 items-start">
          <textarea value={h} onChange={e => { const c = [...items]; c[i] = e.target.value; set(c); }} rows={2} placeholder={placeholder} className="flex-1 border-2 border-gray-100 rounded-xl px-3 py-2 text-sm bg-gray-50 resize-none focus:outline-none focus:border-amber-400 focus:bg-white transition-all" />
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6" style={{ background: DARK }}>
      <div className="w-full max-w-sm">
        ...
        <h1 className="text-white font-extrabold text-2xl text-center mb-2">Inspecciones Eléctricas</h1>
        <p className="text-white/40 text-sm text-center mb-10">Sistema de verificación NEC 2020</p>

        <button onClick={handleLogin} disabled={loading}
          className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-50 active:scale-95 transition-all rounded-2xl py-4 px-6 shadow-lg disabled:opacity-60">
          ...
          <span className="font-bold text-gray-800 text-sm">{loading ? "Redirigiendo…" : "Entrar con Microsoft 365"}</span>
        </button>

        <p className="text-white/20 text-xs text-center mt-6">
          Usa tu cuenta de Electrizar Electromecánica
        </p>
      </div>
    </div>
  );
        ...
        <div className="flex items-center gap-1.5">
          {TOOLS.map(t => (
            <button key={t.id} onClick={() => setTool(t.id)}
              ...
              style={tool === t.id ? { background: Y } : {}}><span className="text-sm leading-none">{t.icon}</span><span className="text-[9px]">{t.L}</span></button>
          ))}
...
/* ═══════════════════════════════════════════
   FINALIZAR MODAL
═══════════════════════════════════════════ */
function FinalizarModal({ informe, onConfirm, onClose, token }) {
  ...
  const handleConfirm = async () => {
    ...
    if (token) {
      try {
        ...
        await oneDrive.saveReport(token, updated);
        setSaved(true);
      } catch (e) { console.warn("OneDrive error:", e); }
    }
    setSaving(false);
    onConfirm(data);
  };
    ...
    <div className="fixed inset-0 bg-black/80 z-50 flex flex-col justify-end sm:justify-center sm:items-center sm:p-4">
      <div className="bg-white w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl overflow-hidden flex flex-col" style={{ maxHeight: "95vh" }}>
        <div className="px-5 pt-5 pb-4 border-b border-gray-100">
          ...
          <p className="text-xs text-gray-400">Código: <span className="font-mono font-bold text-gray-600">{informe.codigo}</span></p>
        </div>
        <div className="overflow-y-auto p-5 space-y-5 pb-6">
          {/* Resumen */}
          <div className="rounded-2xl border-2 border-gray-100 overflow-hidden">
            <div className="px-4 py-3 bg-gray-50 border-b border-gray-100"><p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Resumen</p></div>
          ...
          {token && (
            <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl" style={{ background: "#f0fdf4" }}>
              ...
              <p className="text-xs text-green-700 font-medium">
                {saved ? "✅ Guardado en OneDrive" : "Las fotos y datos se guardarán en OneDrive al confirmar"}
              </p>
            </div>
          )}
...
/* ═══════════════════════════════════════════
   HOME
═══════════════════════════════════════════ */
function HomeView({ informes, onNew, onOpen, user, onLogout }) {
        ...
        <div className="flex items-center justify-between max-w-2xl mx-auto">
          ...
          <div className="flex items-center gap-2">
            <div className="text-right mr-1">
              <div className="text-white text-xs font-bold leading-tight">{user?.displayName || user?.name || "Usuario"}</div>
...
/* ═══════════════════════════════════════════
   NUEVO INFORME
═══════════════════════════════════════════ */
function NuevoView({ onCreate, onBack, user }) {
  const [f, setF] = useState({
    codigo: "", propietario: "", direccion: "", fecha: HOY,
    ingeniero: user?.displayName || user?.name || "",
    matricula: "",
  });
      ...
      <div className="p-4 max-w-lg mx-auto space-y-3 pb-24">
        ...
        <div className="flex items-start gap-3 px-4 py-3.5 rounded-2xl border-2" style={{ borderColor: Y + "40", background: Y + "0f" }}>
          ...
          <p className="text-xs leading-relaxed" style={{ color: "#92400e" }}>
            El <b>resultado de la verificación</b> se solicitará al finalizar la inspección. Las fotos se guardarán automáticamente en OneDrive.
          </p>
        </div>
...
/* ═══════════════════════════════════════════
   INSPECCIÓN ACTIVA
═══════════════════════════════════════════ */
function InspeccionView({ informe, onUpdate, onBack, onFinalize, onPreview, token }) {
    ...
    <div className="min-h-screen" style={{ background: "#f8f9fb" }}>
      ...
      <div className="bg-white border-b border-gray-100 px-4 py-2">
        <div className="max-w-2xl mx-auto flex gap-5 text-xs">
          ...
          {token && <span className="text-green-500 ml-auto">☁️ OneDrive</span>}
        </div>
      </div>
          ...
          <div className="p-4">
            {informe.resultado ? (
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mb-4">Al finalizar se asignará el resultado y se guardará en OneDrive.</p>
      ...
      {finalM && <FinalizarModal informe={informe} onConfirm={handleFinalize} onClose={() => setFinalM(false)} token={token} />}
    </div>
  );
}
      ...
      <div className="min-h-screen" style={{ background: "#e5e7eb" }}>
        ...
        <div className="py-6 px-3 max-w-3xl mx-auto space-y-4">
          ...
          <RPage page={2} total={total} nEl={nEl}>
            <h2 className="font-extrabold text-gray-800 mb-3">Análisis y Limitaciones:</h2>
...
export default function App() {
  ...
  /* Detectar token de retorno OAuth */
  useEffect(() => {
    const existing = authHelper.getToken();
    if (existing) {
      setToken(existing);
      const u = sto.get("ms_user");
      if (u) { setUser(u); setV("home"); return; }
      // Fetch user info
      fetch("https://graph.microsoft.com/v1.0/me", { headers: { Authorization: `Bearer ${existing}` } })
        .then(r => r.json()).then(u => { sto.set("ms_user", u); setUser(u); setV("home"); }).catch(() => setV("home"));
      return;
    }
    const parsed = authHelper.parseToken();
    if (parsed) {
      setToken(parsed);
      fetch("https://graph.microsoft.com/v1.0/me", { headers: { Authorization: `Bearer ${parsed}` } })
        .then(r => r.json()).then(u => { sto.set("ms_user", u); setUser(u); setV("home"); }).catch(() => setV("home"));
    }
  }, []);
  ...
  const logout = () => { authHelper.logout(); sto.set("ms_user", null); setToken(null); setUser(null); setV("login"); };

  if (view === "login") return <LoginView onLogin={() => {}} />;
  if (view === "home") return <HomeView informes={informes} onNew={() => setV("nuevo")} onOpen={open} user={user} onLogout={logout} />;
  if (view === "nuevo") return <NuevoView onCreate={create} onBack={() => setV("home")} user={user} />;
  if (view === "insp") return <InspeccionView informe={active} onUpdate={update} onBack={() => setV("home")} onFinalize={finalize} onPreview={() => setV("prev")} token={token} />;
