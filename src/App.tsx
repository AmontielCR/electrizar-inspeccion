...
const processPhoto = (file) => new Promise((res, rej) => {
...
const generatePDFElectrico = (informe) => {
  ...
  const portada = `<div class="pg cover">
    ...
    <div class="cover-table">
      <div class="cover-row"><div class="cover-lbl">Propietario:</div><div class="cover-val">${informe.propietario}</div></div>
      <div class="cover-row"><div class="cover-lbl">Dirección:</div><div class="cover-val">${informe.direccion || "—"}</div></div>
      <div class="cover-row"><div class="cover-lbl">Fecha:</div><div class="cover-val">${fmtFecha(informe.fecha)}</div></div>
      <div class="cover-row"><div class="cover-lbl">Versión:</div><div class="cover-val">0.1V</div></div>
      ${informe.ingeniero ? `<div class="cover-row"><div class="cover-lbl">Responsable:</div><div class="cover-val">${informe.ingeniero}${informe.matricula ? ". " + informe.matricula + "." : ""}</div></div>` : ""}
    </div>
    ${ftr(1, totalPags)}
  </div>`;

  const limitaciones = `<div class="pg">${hdr}<div class="lim">
    <h2>Análisis y Limitaciones del presente informe fotográfico:</h2>
    <p>El presente informe fotográfico documenta las no conformidades identificadas durante la verificación visual de las instalaciones eléctricas, conforme a los criterios establecidos en el artículo 5.2 del Reglamento de Oficialización del Código Eléctrico de Costa Rica para la Seguridad de la Vida y de la Propiedad (RTCR 458:2011), Decreto Ejecutivo N.° 36979-MEIC y sus reformas. La evaluación se realiza con base en las disposiciones aplicables del Anexo B para condiciones de "Peligro Inminente" o "Alto Riesgo", complementadas con las referencias técnicas correspondientes del Código Eléctrico NFPA 70 (NEC), edición 2020.</p>
    <p>Los hallazgos consignados en este documento se sustentan en la evidencia visible recopilada al momento de la inspección y deben interpretarse como una referencia técnica de las condiciones observadas, no como una delimitación absoluta del alcance correctivo. En consecuencia, durante la ejecución de las reparaciones, toda condición adicional que represente incumplimiento normativo deberá ser corregida igualmente, aun cuando no haya sido señalada de manera individual en este informe.</p>
    <p>Para efectos de la presente verificación, no se contó con planos eléctricos actualizados y debidamente inscritos ante el CFIA, ni con documentación técnica suficiente que permitiera validar integralmente la configuración del sistema eléctrico existente.</p>
  </div>${ftr(2, totalPags)}</div>`;

  const elementosPags = (informe.elementos || []).map((el, i) => {
    ...
    const hh = (el.hallazgos || []).map(h => `<div class="item-line">- ${h}</div>`).join("") || `<div class="item-line" style="color:#aaa">Sin hallazgos registrados.</div>`;
    const aa = (el.acciones  || []).map(a => `<div class="item-line">- ${a}</div>`).join("") || `<div class="item-line" style="color:#aaa">Sin acciones registradas.</div>`;
    return `<div class="pg">${hdr}
      <div class="el-row">
        <div><div style="flex-shrink:0">${img}</div><div class="el-num">(${el.num})</div></div>
        <div class="el-text">
          <div class="el-title-row"><span class="el-title">Título: ${el.titulo}</span><span class="badge ${bc}">${bl}</span></div>
          <div class="sec-lbl">Hallazgos:</div>${hh}
          <div class="sec-lbl">Acciones requeridas:</div>${aa}
        </div>
      </div>
      ${ftr(i + 3, totalPags)}
    </div>`;
  ...
  const cierre = `<div class="pg">${hdr}<div class="close">
    ...
    ${informe.ingeniero ? `<div class="sig-line"></div><div style="font-size:10pt;font-weight:700">${informe.ingeniero}</div><div style="font-size:9pt;color:#555">${informe.matricula || ""}</div>` : ""}
  </div>${ftr(totalPags, totalPags)}</div>`;

  const html = `<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"/><title>${informe.codigo}</title><style>${css}</style></head><body>${portada}${limitaciones}${elementosPags}${cierre}</body></html>`;
...
/* ─────────────────────────────────────────
   PDF — AIRES ACONDICIONADOS (portrait carta)
───────────────────────────────────────── */
const generatePDFAC = (informe) => {
  ...
  const css = `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .cover-val   { color:#333; }
    /* resumen */
    .res-table { width:100%; border-collapse:collapse; margin-top:10px; font-size:8.5pt; }
  ...
  const portada = `<div class="pg cover">
    ...
    <div class="cover-table">
      <div class="cover-row"><div class="cover-lbl">Cliente:</div><div class="cover-val">${informe.propietario}</div></div>
      <div class="cover-row"><div class="cover-lbl">Dirección:</div><div class="cover-val">${informe.direccion || "—"}</div></div>
      <div class="cover-row"><div class="cover-lbl">Código:</div><div class="cover-val">${informe.codigo}</div></div>
      <div class="cover-row"><div class="cover-lbl">Fecha:</div><div class="cover-val">${fmtFecha(informe.fecha)}</div></div>
      <div class="cover-row"><div class="cover-lbl">Versión:</div><div class="cover-val">0.1V</div></div>
      ${informe.tecnico ? `<div class="cover-row"><div class="cover-lbl">Técnico responsable:</div><div class="cover-val">${informe.tecnico}</div></div>` : ""}
    </div>
    ${ftr(1, totalPags)}
  </div>`;
  ...
  const resumen = `<div class="pg">${hdr}
    ...
    <div class="sum-row">
      ...
      <div class="sum-box"><div class="sum-num" style="color:#16a34a">${ap}</div><div class="sum-lbl">Aprobados</div></div>
      <div class="sum-box"><div class="sum-num" style="color:#dc2626">${noAp}</div><div class="sum-lbl">No Aprobados</div></div>
    </div>
    ${ftr(2, totalPags)}
  </div>`;

  const equiposPags = equipos.map((eq, i) => {
    const foto = (key, label, icon) => {
      const src = eq.fotos?.[key];
      return `<div class="foto-cell"><div class="foto-lbl">${icon} ${label}</div>${src ? `<img class="foto-img" src="${src}" alt="${label}"/>` : `<div class="foto-empty">📷</div>`}</div>`;
    };
    ...
    return `<div class="pg">${hdr}
      ...
      <div class="foto-grid">
        ${foto("evaporadora",  "Limpieza de evaporadora",         "❄️")}
        ${foto("condensadora", "Limpieza de condensadora",         "💧")}
        ${foto("tuberia",      "Estado de tubería y aislante",     "🔧")}
        ${foto("presiones",    "Nivel de presión de refrigerante", "🔵")}
      </div>
      ${eq.resultado === "No Aprobado" ? `<div class="sec-lbl">Acciones realizadas / requeridas:</div>${acHTML}` : ""}
      ${eq.observaciones ? `<div class="sec-lbl">Observaciones:</div><div class="item-line">${eq.observaciones}</div>` : ""}
      ${ftr(i + 3, totalPags)}
    </div>`;
  }).join("");
...
const Btn = ({ children, onClick, sm, disabled, full, className = "" }) => {
  ...
  return <button onClick={onClick} disabled={disabled} className={`${base} ${disabled ? "opacity-50 cursor-not-allowed" : "hover:opacity-90"}`} style={{ background: disabled ? "#9ca3af" : Y, color: "white" }}>{children}</button>;
};
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6" style={{ background: DARK }}>
      <div className="w-full max-w-sm">
        ...
        <div className="space-y-3">
          <div>
            ...
            <input value={user} onChange={e => { setU(e.target.value); setE(null); }} onKeyDown={e => e.key === "Enter" && handle()}
              className="w-full bg-white/10 border-2 border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/30 text-sm focus:outline-none focus:border-amber-400 transition-all" placeholder="amontiel"/>
          </div>
          <div>
        ...
        <p className="text-white/20 text-xs text-center mt-8">📵 Funciona sin conexión · 🔄 Sincroniza automáticamente</p>
      </div>
    </div>
  );
}
          ...
          <button onClick={() => onSelect("ac")} className="w-full rounded-2xl p-5 text-left hover:opacity-90 active:scale-95 transition-all shadow-xl border-2" style={{ background: "#0c4a6e", borderColor: "#38bdf8aa" }}>
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <div className="text-xs text-white/60 leading-relaxed">Mantenimiento preventivo de equipos de aire acondicionado. 4 fotos por equipo.</div>
...
function FinalizarModal({ informe, onConfirm, onClose }) {
  ...
  const ress = tipo === "inspeccion" ? RES_INSP : RES_VER;
...
/* ─────────────────────────────────────────
   HOME VIEW — tabs + indicador online/offline
───────────────────────────────────────── */
        ...
        <div className="max-w-2xl mx-auto mt-3 flex gap-1.5 overflow-x-auto pb-0.5">
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-extrabold ${tabActiva === t.id ? "bg-black/20 text-gray-900" : "bg-white/10 text-white/60"}`}>{conteo(t.id)}</span>
            </button>
          ))}
        </div>
      </header>
      ...
      <div className="p-4 max-w-2xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            <div className="w-20 h-20 rounded-2xl mx-auto mb-5 flex items-center justify-center text-4xl" style={{ background: Y + "22" }}>
              {TABS.find(t => t.id === tabActiva)?.icon}
            </div>
            <p className="text-gray-400 text-sm mb-6">
              {tabActiva === "todos" ? "No hay informes. Toca + Nuevo para comenzar." : `No hay informes de ${TABS.find(t => t.id === tabActiva)?.label}.`}
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {filtrados.map(inf => {
              ...
              const apAC = inf.tipo === "ac" ? (inf.equipos || []).filter(e => e.resultado === "Aprobado").length : null;
                ...
                <div key={inf.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  ...
                  <div onClick={() => onOpen(inf)} className="p-4 cursor-pointer hover:bg-gray-50 active:bg-gray-100 transition-colors">
                    <div className="flex justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap mb-0.5">
                          ...
                          {tabActiva === "todos" && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: tb.bg, color: tb.color }}>{tb.label}</span>}
                    ...
                    <div className="flex items-center gap-4 mt-3 pt-3 border-t border-gray-50 text-xs text-gray-400">
                      {inf.tipo === "ac"
                        ? <><span>🌡️ <b className="text-gray-600">{nEl}</b> equipos</span>{apAC !== null && <span className="text-green-500 font-bold">✅ {apAC}/{nEl}</span>}</>
                        : <><span>📷 <b className="text-gray-600">{nEl}</b> elementos</span>{altaCount > 0 && <span className="font-bold text-red-400">🔴 {altaCount} alta prioridad</span>}</>}
  ...
  return (
    <div className="min-h-screen" style={{ background: "#f8f9fb" }}>
      ...
      <div className="p-4 max-w-lg mx-auto space-y-3 pb-24">
        ...
        <Btn onClick={go} full>{tipo === "ac" ? "Continuar → Agregar equipos" : "Iniciar →"}</Btn>
      </div>
    </div>
  );
}
...
function EquipoACModal({ equipo, num, onSave, onClose }) {
  ...
  const onPhoto = async (e, key) => { const f = e.target.files[0]; if (!f) return; try { const { url } = await processPhoto(f); setAnn({ url, key }); } catch { setErr("Error al procesar la imagen."); } };
  const onAnnotConfirm = (finalUrl) => { setFotos(p => ({ ...p, [annot.key]: finalUrl })); setAnn(null); };
  const save = () => {
                ...
                <div key={f.key}>
                  ...
                  {fotos[f.key] ? (
                    <div className="relative">
                      ...
                      <button onClick={() => fileRefs.current[f.key].click()} className="absolute bottom-2 right-2 px-2.5 py-1.5 rounded-lg bg-black/60 text-white text-[10px] font-bold">📷 Cambiar</button>
                    </div>
                  ) : (
          ...
          <div>
            ...
            <div className="flex gap-3">
              <button onClick={() => { setRes("Aprobado"); setErr(null); }}
                ...
                style={resultado === "Aprobado" ? { background: "#16a34a" } : {}}>✅ Aprobado</button>
              <button onClick={() => { setRes("No Aprobado"); setErr(null); }}
                ...
                style={resultado === "No Aprobado" ? { background: "#dc2626" } : {}}>❌ No Aprobado</button>
            </div>
          </div>
...
function MantenimientoACView({ informe, onUpdate, onBack, onFinalize }) {
  ...
  const handleFinalizar = async () => {
    ...
    setSyncing(true); await saveInformeFS(updated); setSyncing(false); onFinalize(updated);
  };
      ...
      <header style={{ background: "#0c4a6e" }} className="px-4 py-4 sticky top-0 z-10 shadow-lg">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex items-start justify-between ml-1">
            ...
            {informe.resultado && <button onClick={() => generatePDFAC(informe)} className="px-3 py-1.5 rounded-xl text-xs font-bold border-2 border-white/20 text-white hover:bg-white/10 transition-colors">🖨️ PDF</button>}
          </div>
        </div>
      </header>
      ...
      <div className="p-4 max-w-2xl mx-auto pb-36 space-y-3">
        {!equipos.length ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 text-sm">Toca <b style={{ color: Y }}>+</b> para agregar el primer equipo</p>
          </div>
        ) : (
          equipos.map((eq, i) => (
                ...
                <div className="flex gap-1.5 mt-2">
                  {AC_FOTOS.map(f => (
                    <div key={f.key} className="flex-1 aspect-square rounded-lg overflow-hidden border border-gray-100" style={{ maxWidth: 60 }}>
                      {eq.fotos?.[f.key] ? <img src={eq.fotos[f.key]} alt={f.label} className="w-full h-full object-cover"/> : <div className="w-full h-full bg-gray-100 flex items-center justify-center text-gray-300 text-sm">{f.icon}</div>}
                    </div>
                  ))}
                </div>
                {eq.resultado === "No Aprobado" && eq.acciones?.length > 0 && <div className="mt-2 text-xs text-red-600 font-medium">⚠️ {eq.acciones.length} acción{eq.acciones.length > 1 ? "es" : ""} reportada{eq.acciones.length > 1 ? "s" : ""}</div>}
              </div>
              <div className="flex border-t border-gray-50 divide-x divide-gray-50 text-xs">
          ...
          <div className="p-4">
            {informe.resultado ? (
              ...
              <div>
                ...
                <button onClick={handleFinalizar} className="w-full py-3.5 rounded-xl font-extrabold text-white text-sm hover:opacity-90 active:scale-95 transition-all" style={{ background: "#0c4a6e" }}>
...
function ElementoModal({ el, num, onSave, onClose }) {
  ...
  const analyze = async (b = b64) => {
    if (!b) { setErr("Primero adjunta foto."); return; }
    if (!titulo.trim()) { setErr("Escribe el título."); return; }
...
function InspeccionView({ informe, onUpdate, onBack, onFinalize }) {
  ...
  const [syncing, setSyncing] = useState(false);
  const tipo      = informe.tipo || "verificacion";
  const tipoLabel = tipo === "inspeccion" ? "Inspección" : "Verificación";
    ...
    <div className="min-h-screen" style={{ background: "#f8f9fb" }}>
      <header style={{ background: DARK }} className="px-4 py-4 sticky top-0 z-10 shadow-lg">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex items-start justify-between ml-1">
            <div>
              <div className="font-mono text-[11px] text-white/40">{informe.codigo}</div>
              <div className="font-bold text-white text-sm">{informe.propietario}</div>
            </div>
            <div className="flex items-center gap-2">
      ...
      <div className="p-4 max-w-2xl mx-auto pb-36 space-y-3">
        {!total ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 text-sm">Toca <b style={{ color: Y }}>+</b> para agregar el primer elemento</p>
          </div>
        ) : (
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q,
      (snap) => { setI(snap.docs.map(d => ({ ...d.data(), id: d.id }))); setLoad(false); },
