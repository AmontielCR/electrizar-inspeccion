...
{"hallazgos":["hallazgo 1 citando Art. NEC correspondiente"],"acciones":["acción correctiva 1"]}

Reglas:
- Cita artículo NEC 2020 en CADA hallazgo (ej: Art. 110.12(A))
- Máximo 5 hallazgos y 5 acciones
- Español técnico
...
const Logo = ({ sm, white }) => (
  <div className="flex items-center gap-2">
    <div className={`${sm?"w-7 h-7":"w-10 h-10"} flex items-center justify-center rounded-md flex-shrink-0`}
         style={{ background: Y }}>
    ...
    <div>
      <div className={`font-bold leading-tight ${sm?"text-xs":"text-sm"} ${white?"text-white":"text-gray-900"}`}>
        Electrizar
      </div>
      <div className={`leading-tight ${sm?"text-[9px]":"text-[10px]"}`} style={{color:Y}}>
...
const Badge = ({ v }) => {
  const p = PRIOS.find(x=>x.v===v)||PRIOS[1];
  return <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${p.tw}`}>{p.L}</span>;
};

const ResBadge = ({ v }) => {
  if (!v) return null;
  const cfg = v==="Aprobada"  ? "bg-green-100 text-green-700" :
              v==="Rechazada" ? "bg-red-100 text-red-700"     :
...
const Pill = ({ children, active, color, onClick }) => (
  <button onClick={onClick}
    className={`flex-1 py-2.5 rounded-xl text-sm font-bold border-2 transition-all ${
      active ? "text-white" : "border-gray-200 text-gray-400 bg-white hover:bg-gray-50"}`}
    style={active ? { background:color||Y, borderColor:color||Y } : {}}>
    {children}
  </button>
);

const Btn = ({ children, onClick, variant="pri", sm, disabled, full, danger, className="" }) => {
  const base = `font-bold rounded-xl transition-all active:scale-95 flex items-center justify-center gap-1.5
    ${sm?"px-3 py-2 text-sm":"px-5 py-3"} ${full?"w-full":""} ${className}`;
  if (variant==="pri")
    return <button onClick={onClick} disabled={disabled}
      className={`${base} ${disabled?"opacity-50 cursor-not-allowed":"hover:opacity-90"}`}
      style={{ background:disabled?"#9ca3af":danger?"#dc2626":Y, color:"white" }}>{children}</button>;
  return <button onClick={onClick} disabled={disabled}
    className={`${base} border-2 border-gray-200 bg-white text-gray-700 hover:bg-gray-50 ${disabled?"opacity-50":""}`}>
    {children}</button>;
};
  ...
  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    {title && (
      <div className="px-4 pt-4 pb-2 flex items-center gap-2">
        {accent && <div className="w-1 h-4 rounded-full" style={{background:Y}}/>}
        <h3 className="font-bold text-gray-700 text-xs uppercase tracking-widest">{title}</h3>
      </div>
    )}
...
const FI = ({ label, value, onChange, ...rest }) => (
  <div>
    ...
    <input value={value} onChange={onChange}
      className="w-full border-2 border-gray-100 rounded-xl px-3 py-2.5 text-sm text-gray-800 bg-gray-50
  ...
  <div>
    <div className="flex items-center justify-between mb-2">
      ...
      <button onClick={()=>set([...items,""])}
        ...
        style={{color:Y, background:Y+"18"}}>+ Agregar</button>
    </div>
    <div className="space-y-2">
      {!items.length && (
        <div className="text-xs text-gray-400 text-center py-4 border-2 border-dashed border-gray-100 rounded-xl">
          Sin {label.toLowerCase()} — usa IA o toca Agregar
        </div>
      )}
      {items.map((h,i)=>(
        <div key={i} className="flex gap-2 items-start">
          <textarea value={h} onChange={e=>{const c=[...items];c[i]=e.target.value;set(c);}}
            rows={2} placeholder={placeholder}
            className="flex-1 border-2 border-gray-100 rounded-xl px-3 py-2 text-sm bg-gray-50 resize-none
                       focus:outline-none focus:border-amber-400 focus:bg-white transition-all"/>
          <button onClick={()=>set(items.filter((_,j)=>j!==i))}
            className="mt-1.5 w-6 h-6 flex items-center justify-center rounded-full
                       text-gray-300 hover:text-red-400 hover:bg-red-50 transition-all text-base font-bold">×</button>
        </div>
      ))}
    </div>
  </div>
        ...
        <div className="px-5 pt-5 pb-4 border-b border-gray-100">
          <div className="flex items-center justify-between mb-1">
            ...
            <button onClick={onClose}
              className="w-8 h-8 flex items-center justify-center rounded-xl text-gray-300 hover:text-gray-600 hover:bg-gray-100 font-bold text-lg">✕</button>
          </div>
        ...
        <div className="overflow-y-auto p-5 space-y-5 pb-6">
          ...
          <div className="rounded-2xl border-2 border-gray-100 overflow-hidden">
            <div className="px-4 py-3 bg-gray-50 border-b border-gray-100">
              <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Resumen de la inspección</p>
            </div>
            <div className="grid grid-cols-4 divide-x divide-gray-100">
              ...
              <div className="px-3 py-3 text-center">
                <div className="text-xl font-extrabold text-red-500">{alta}</div>
                <div className="text-[10px] text-gray-400 mt-0.5">Alta</div>
              </div>
              <div className="px-3 py-3 text-center">
                <div className="text-xl font-extrabold text-amber-500">{normal}</div>
                <div className="text-[10px] text-gray-400 mt-0.5">Normal</div>
              </div>
              <div className="px-3 py-3 text-center">
                <div className="text-xl font-extrabold text-green-500">{baja}</div>
                <div className="text-[10px] text-gray-400 mt-0.5">Baja</div>
              </div>
            </div>
          </div>
          ...
          {/* Sugerencia */}
          {!informe.resultado && (
            <div className="flex items-start gap-3 px-4 py-3 rounded-2xl border-2"
                ...
                return (
                  <button key={r} onClick={()=>setRes(r)}
                    ...
                    <div className="w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-all"
                         style={{
                           borderColor: active ? c.color : "#d1d5db",
          ...
          {resultado !== "Aprobada" && (
            <div>
              <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">
                Plazo para ejecución de mejoras
              </label>
              <div className="flex gap-2 flex-wrap mb-2">
                {["3 meses","6 meses","12 meses","18 meses","24 meses"].map(p=>(
                  <button key={p} onClick={()=>setPlazo(p)}
                    ...
                    style={{
                      borderColor: plazo===p ? Y : "#e5e7eb",
                      background:  plazo===p ? Y+"15" : "white",
                      color:       plazo===p ? Y : "#6b7280",
                    }}>{p}</button>
                ))}
              </div>
          ...
          <div>
            ...
            <textarea value={notas} onChange={e=>setNotas(e.target.value)} rows={3}
              ...
              className="w-full border-2 border-gray-100 rounded-xl px-3 py-2.5 text-sm bg-gray-50 resize-none
                         focus:outline-none focus:border-amber-400 focus:bg-white transition-all"/>
          </div>

          {/* CTA */}
          <button onClick={()=>onConfirm({resultado, plazo, notas})}
            className="w-full py-4 rounded-2xl font-extrabold text-white text-base hover:opacity-90 active:scale-95 transition-all"
            style={{
              background: resultado==="Aprobada"  ? "#16a34a" :
                          resultado==="Rechazada" ? "#dc2626" : Y,
            }}>
            Generar Reporte {resultado} →
          </button>

          <p className="text-center text-xs text-gray-400">
            Podrás editar el resultado y las notas antes de imprimir el PDF.
          </p>
        </div>
      </div>
    </div>
...
function AnnotationEditor({ imageUrl, onConfirm, onClose }) {
  ...
  useEffect(()=>{
    ...
    img.onload=()=>{
      ...
      c.width=img.naturalWidth; c.height=img.naturalHeight; setLoaded(true);
    };
    img.src=imageUrl;
  },[imageUrl]);
  ...
  const drawOne=useCallback((ctx,s)=>{
    ...
    if (s.tool==="arrow"){
      ...
      ctx.beginPath();
      ctx.moveTo(s.x1-sz,s.y1-sz);ctx.lineTo(s.x1+sz,s.y1+sz);
      ctx.moveTo(s.x1+sz,s.y1-sz);ctx.lineTo(s.x1-sz,s.y1+sz);
      ctx.stroke();
    } else if (s.tool==="text"){
      ...
      ctx.fillStyle=s.color;ctx.fillText(s.text,s.x1,s.y1);
    }
    ctx.restore();
  },[]);
  ...
  const addText=()=>{
    if (textVal.trim()&&textDlg)
      setShapes(p=>[...p,{tool:"text",color,x1:textDlg.x,y1:textDlg.y,text:textVal.trim()}]);
  ...
  const doConfirm=()=>{
    ...
    onConfirm(dUrl,dUrl.split(",")[1]);
  };

  const TOOLS=[
    {id:"arrow", icon:"↗",L:"Flecha"},
    {id:"circle",icon:"○",L:"Círculo"},
    {id:"rect",  icon:"□",L:"Rect."},
    {id:"cross", icon:"✕",L:"Cruz"},
    {id:"text",  icon:"T",L:"Texto"},
  ];
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col select-none">
      <div className="flex items-center justify-between px-3 py-2.5 shrink-0" style={{background:DARK}}>
        ...
        <button onClick={doConfirm} className="px-4 py-1.5 rounded-xl font-bold text-sm text-gray-900 hover:opacity-90" style={{background:Y}}>✓ Confirmar</button>
      </div>
        ...
        {textDlg && (
          <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-10 p-4">
            <div className="bg-white rounded-2xl p-5 w-full max-w-sm shadow-2xl">
              ...
              <input autoFocus value={textVal} onChange={e=>setTVal(e.target.value)}
                ...
                placeholder="Ej: Sin conductor de tierra…"/>
        ...
        <div className="flex items-center gap-1.5">
          {TOOLS.map(t=>(
            <button key={t.id} onClick={()=>setTool(t.id)}
              className={`flex-1 py-2 rounded-xl flex flex-col items-center gap-0.5 text-xs font-bold border-2 transition-all ${
                tool===t.id?"border-transparent text-gray-900":"border-white/10 text-white/50 hover:text-white"}`}
              style={tool===t.id?{background:Y}:{}}>
              <span className="text-sm leading-none">{t.icon}</span>
              <span className="text-[9px]">{t.L}</span>
            </button>
          ))}
          <div className="w-px h-10 bg-white/10 mx-0.5"/>
          <button onClick={()=>setShapes(p=>p.slice(0,-1))}
            className="px-2.5 py-2 rounded-xl border-2 border-white/10 text-white/50 hover:text-white hover:bg-white/10 flex flex-col items-center">
            <span className="text-sm">↩</span><span className="text-[9px] text-white/40">Undo</span>
          </button>
          <button onClick={()=>{if(confirm("¿Borrar todas las anotaciones?"))setShapes([]);}}
            className="px-2.5 py-2 rounded-xl border-2 border-white/10 text-white/50 hover:text-red-400 hover:bg-red-900/30 flex flex-col items-center">
            <span className="text-sm">🗑️</span><span className="text-[9px] text-white/40">Clear</span>
          </button>
        </div>
          ...
          {COLS.map(c=>(
            <button key={c} onClick={()=>setColor(c)}
              ...
              style={{
                ...
                outline:color===c?`3px solid ${Y}`:"3px solid transparent",
                outlineOffset:2,
                border:c==="#ffffff"?"2px solid rgba(255,255,255,0.25)":"none",
              }}/>
          ))}
...
/* ═══════════════════════════════════════════
   HOME
═══════════════════════════════════════════ */
function HomeView({ informes, onNew, onOpen }) {
  ...
  return (
    <div className="min-h-screen" style={{background:"#f8f9fb"}}>
      ...
      <div className="p-4 max-w-2xl mx-auto">
        <h2 className="font-bold text-gray-400 text-xs uppercase tracking-widest mb-4 mt-1">Informes ({informes.length})</h2>
        {!sorted.length ? (
          ...
          <div className="space-y-3">
            {sorted.map(inf=>{
              ...
              const finalizado=!!inf.resultado;
              return (
...
/* ═══════════════════════════════════════════
   NUEVO INFORME  (sin resultado ni plazo)
═══════════════════════════════════════════ */
function NuevoView({ onCreate, onBack }) {
  const [f,setF]=useState({codigo:"",propietario:"",direccion:"",fecha:HOY,ingeniero:"",matricula:""});
      ...
      <div className="p-4 max-w-lg mx-auto space-y-3 pb-24">
        ...
        <div className="flex items-start gap-3 px-4 py-3.5 rounded-2xl border-2" style={{borderColor:Y+"40",background:Y+"0f"}}>
          ...
          <p className="text-xs leading-relaxed" style={{color:"#92400e"}}>
            El <b>resultado de la verificación</b> (Aprobada / Condicionada / Rechazada) se solicitará al finalizar la inspección, una vez completados todos los elementos.
          </p>
        </div>

        <Btn onClick={go} full>Iniciar Inspección →</Btn>
        <p className="text-center text-xs text-gray-400">Los datos se guardan en este dispositivo.</p>
      </div>
    </div>
  );
}
...
function InspeccionView({ informe, onUpdate, onBack, onFinalize, onPreview }) {
  ...
  const alta  =(informe.elementos||[]).filter(e=>e.prioridad==="alta").length;
  const total =informe.elementos?.length||0;
  const finalizado=!!informe.resultado;
    ...
    <div className="min-h-screen" style={{background:"#f8f9fb"}}>
      <header style={{background:DARK}} className="px-4 py-4 sticky top-0 z-10 shadow-lg">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex items-start justify-between ml-1">
            ...
            <div className="flex items-center gap-2">
              {finalizado && <ResBadge v={informe.resultado}/>}
              {finalizado && <Btn onClick={onPreview} sm variant="out">📄 PDF</Btn>}
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="bg-white border-b border-gray-100 px-4 py-2">
        <div className="max-w-2xl mx-auto flex gap-5 text-xs">
          ...
          {!finalizado && <span className="font-bold text-blue-500 ml-auto">● En curso</span>}
        </div>
      </div>
      ...
      <div className="p-4 max-w-2xl mx-auto pb-36 space-y-3">
          ...
          informe.elementos.map((el,i)=>(
            <div key={i} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="flex gap-3 p-3">
                {el.url
                  ?<img src={el.url} alt={el.titulo} className="w-20 h-20 object-cover rounded-xl shrink-0"/>
                  :<div className="w-20 h-20 bg-gray-100 rounded-xl shrink-0 flex items-center justify-center text-2xl">📷</div>
                }
        ...
        {/* Finalizar card — siempre visible */}
        <div className={`rounded-2xl border-2 overflow-hidden ${finalizado?"border-green-200 bg-green-50":"border-dashed border-gray-200 bg-white"}`}>
          <div className="p-4">
            {finalizado ? (
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button onClick={()=>setFinalM(true)}
                    className="px-3 py-1.5 rounded-xl text-xs font-bold border-2 border-green-200 text-green-700 hover:bg-green-100 transition-colors">
                    Editar
                  </button>
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mb-4">Al finalizar se te pedirá el resultado de la verificación y se generará el reporte.</p>
                <button onClick={()=>{
                  if (total===0) { alert("Agrega al menos un elemento antes de finalizar."); return; }
                  setFinalM(true);
                }}
      ...
      <button onClick={()=>{setEditIdx(null);setModal(true);}}
        className="fixed bottom-6 right-6 w-14 h-14 rounded-2xl shadow-2xl text-white text-3xl
                   flex items-center justify-center z-20 active:scale-90 hover:opacity-90 transition-all"
        style={{background:Y}}>+</button>

      {modal && (
        <ElementoModal
          el={editIdx!==null?informe.elementos[editIdx]:null}
          num={editIdx!==null?editIdx+1:(informe.elementos?.length||0)+1}
          onSave={saveEl} onClose={()=>setModal(false)}/>
      )}
...
function ElementoModal({ el, num, onSave, onClose }) {
  ...
  const onPhoto=async(e)=>{
    ...
    setErr(null);
    try { const {url:u}=await processPhoto(f); setAnn(u); }
    catch { setErr("Error al procesar la imagen."); }
  };

  const onAnnotConfirm=(finalUrl,finalB64)=>{
    setUrl(finalUrl); setB64(finalB64); setAnn(null);
    if (titulo.trim()) analyze(finalB64);
    else setErr("Foto guardada. Escribe el título y toca Analizar IA.");
  };
    ...
    <div className="fixed inset-0 bg-black/70 z-40 flex flex-col justify-end sm:justify-center sm:items-center sm:p-4">
      <div className="bg-white w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl overflow-hidden flex flex-col" style={{maxHeight:"93vh"}}>
        ...
        <div className="overflow-y-auto p-4 space-y-4 pb-6">
            ...
            {url ? (
              <div>
                ...
                <div className="grid grid-cols-3 gap-2 mt-2.5">
                  ...
                  <button onClick={()=>setAnn(url)} className="py-2.5 border-2 rounded-xl text-xs font-bold hover:opacity-80"
                    style={{borderColor:Y,color:Y,background:Y+"12"}}>✏️ Anotar</button>
                  <button onClick={()=>analyze()} disabled={busy||!titulo.trim()}
                    className="py-2.5 rounded-xl text-xs text-white font-bold disabled:opacity-50 hover:opacity-90"
                    style={{background:DARK}}>{busy?"⏳…":"🤖 IA NEC"}</button>
                </div>
              </div>
            ) : (
              <button onClick={()=>fRef.current.click()}
                className="w-full border-2 border-dashed border-gray-200 rounded-2xl py-12 text-center hover:border-amber-400 hover:bg-amber-50 transition-all active:scale-95">
          ...
          {busy && (
            <div className="flex items-center justify-center gap-3 py-4 rounded-2xl" style={{background:Y+"15"}}>
              <div className="w-5 h-5 rounded-full border-2 animate-spin" style={{borderColor:Y,borderTopColor:"transparent"}}/>
              <span className="text-sm font-bold" style={{color:Y}}>Analizando con NEC 2020…</span>
            </div>
          )}
          ...
          <div>
            ...
            <div className="flex gap-2">
              {PRIOS.map(p=><Pill key={p.v} active={prio===p.v} color={p.c} onClick={()=>setP(p.v)}>{p.L}</Pill>)}
            </div>
          </div>
        ...
        <div className="py-6 px-3 max-w-3xl mx-auto space-y-4">
          <RPage page={1} total={total} nEl={nEl}>
            <div className="flex flex-col items-center py-8 text-center">
              ...
              <h1 className="text-xl font-extrabold text-gray-900 mb-3 leading-snug max-w-md">
                {informe.codigo} — Informe Verificación Eléctrica
              </h1>
          ...
          <RPage page={2} total={total} nEl={nEl}>
            ...
            <p className="text-xs text-gray-600 leading-relaxed mb-2">
              El presente informe fotográfico documenta las no conformidades identificadas durante la verificación visual de las instalaciones eléctricas, conforme al artículo 5.2 del Reglamento de Oficialización del Código Eléctrico de Costa Rica (RTCR 458:2011), Decreto Ejecutivo N.° 36979-MEIC y sus reformas. La evaluación se fundamenta en el Anexo B para condiciones de "Peligro Inminente" o "Alto Riesgo", complementada con las referencias del Código Eléctrico NFPA 70 (NEC), edición 2020.
            </p>
            <p className="text-xs text-gray-600 leading-relaxed">
              Los hallazgos se sustentan en la evidencia visible al momento de la inspección. Durante la ejecución de reparaciones, toda condición adicional que represente incumplimiento normativo deberá ser corregida, aun cuando no haya sido señalada individualmente.
            </p>
          </RPage>
          {(informe.elementos||[]).map((el,i)=>(
            <RPage key={i} page={i+3} total={total} nEl={nEl}>
              <div className="flex gap-5">
                {el.url
                  ?<img src={el.url} alt={el.titulo} className="w-52 h-48 object-cover rounded-2xl border border-gray-200 shrink-0"/>
                  :<div className="w-52 h-48 bg-gray-100 rounded-2xl shrink-0 flex items-center justify-center text-4xl">📷</div>
                }
                <div className="flex-1 min-w-0">
            ...
            {informe.ingeniero&&(
              <div className="mt-16">
                ...
                <div className="text-sm text-gray-700">
                  <div className="font-bold">{informe.ingeniero}</div>
                  <div>{informe.matricula}</div>
                </div>
              </div>
            )}
          </RPage>
...
export default function App() {
  ...
  const update=(inf)=>{
    const l=informes.map(i=>i.id===inf.id?inf:i);
    persist(l); setAct(inf);
  };

  const finalize=(data)=>{
    const inf={...active,...data};
    update(inf);
    setV("prev");
  };
  ...
  if (view==="home")  return <HomeView informes={informes} onNew={()=>setV("nuevo")} onOpen={open}/>;
  if (view==="nuevo") return <NuevoView onCreate={create} onBack={()=>setV("home")}/>;
